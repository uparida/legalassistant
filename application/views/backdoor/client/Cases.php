<div class="row"  ng-init="setBaseUrl('<?php echo base_url() ?>')">
        ...
        <table class="table table-hover">
            ...
            <tbody>
                ...
                <tr  ng-repeat="x in cases track by $index" ng-class='{green : x.status == "OPENED",red : x.status == "CANCELLED",grey : x.status == "CLOSED",yellow : x.status == "UNDER-REVIEW"}'>
                    ...
                    <td>{{x.answers[0].email}}</td>
                    <td>{{x.status}}</td>
                    <td class="col-sm-2"> <a href="#cases-view/{{x.id}}"><button class="btn btn-warning" type="submit" ng-model="selectedCase" value="{{x.id}}">View</button></a></td>
                </tr>
